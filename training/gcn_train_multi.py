...
from models.gcn_features import GCN_MultiClass as GCN
import multi_spektral.tf_to_spektral as tf_to_s
from multi_spektral.spektral_dataset import MyDataset
from multi_spektral.custom_graph_loader import CustomGraphSequence as CustomLoader
import pandas as pd
...
print(f"Shape za labele: train: {y_train_one_hot.shape}, val: {y_val_one_hot.shape}, test: {y_test_one_hot.shape}")
print(f'Primer jedne labele: {y_train_one_hot[0]}')
...
"""
train_loader = CustomLoader(train_dataset, batch_size=batch_size)
val_loader = CustomLoader(val_dataset, batch_size=batch_size)
test_loader = CustomLoader(test_dataset, batch_size=batch_size)

batch_tuple = next(iter(train_loader))
print(f"Broj elemenata u batch-u: {len(batch_tuple)}")

num_epoch = 50
print("Tri skupa prilagodjena za trening, val i test kreirani da bi mogli pustiti u GCN.")
num_features = 12  # Broj feature-a po čvoru
num_labels = 3     # Broj klasa (podela po aktivnosti u odnosu na ciljani receptor)
hidden_units = 128  # Skriveni slojevi GCN sa 64 jedinice
dense_units = [128, 128]
dropout_rate = 0.26
...
model.compile(tf.keras.optimizers.Adam(learning_rate=0.0009868), #lr po optuna proracunu
...
final_dir = '/home/ivanam/BIO-Info/bio_info/multi_seminar2/training/results_with_features/gcn'
with open(f'{final_dir}/model_summary-gcn-max-e{num_epoch}.txt', 'w') as f:
    model.summary(print_fn=lambda x: f.write(x + '\n'))
print("Zavrseno pisanje u fajl.")

print("Pisemo history treninga..")
history_df = pd.DataFrame(history.history)
history_df.to_csv(f'{final_dir}/gcn_history-max-e{num_epoch}.csv', index=False)

print("Cuvanje tezina modela..")
model.save_weights(f'{final_dir}/gcn_model-max-e{num_epoch}.h5')

output_file_path = f"{final_dir}/gcn_test_results-max-e{num_epoch}.txt"
...
with open(output_file_path, "w") as f:
    ...
    test_loss, test_acc, test_f1_score, test_roc_auc, test_avg_precision = model.evaluate(test_loader, batch_size= batch_size, steps =steps_for_test, verbose=1)
    ...
    for step, batch in enumerate(test_loader):
        ...
        y_pred_batch = model((x, a, i, u), training=False)    
